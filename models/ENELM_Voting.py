...
def train(config):
  ...
  #en_y_pred, _ = torch.median(torch.round(torch.cat(predictions, dim=1) / config.round_to) * config.round_to, dim=1)
  en_y_pred = torch.mean(torch.cat(predictions, dim=1), dim=1)
